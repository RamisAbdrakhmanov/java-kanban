...
abstract class TaskManagerTest<T extends TaskManager> {
    ...
    void addNewTaskSameTimeTest() {
        ...
        Task task = new Task("Test123 addNewTask",
                ...
                "1000");
        taskManager.addTask(task);
        assertFalse(taskManager.getTasks().contains(task), "Некорректное добавление пересекающихся задач");
    }

    @Test
    void addNewTaskNullTest() {
        assertDoesNotThrow(() -> taskManager.addTask(null), "Некорректная обработка null");
    }
    ...
    void changeTaskNullTest() {
        assertDoesNotThrow(() -> taskManager.updateTask(null), "Некорректная обработка null");
    }
    ...
    void deleteAllTaskTest() {
        ...
        assertEquals(new ArrayList<>(), taskManager.getTasks(), "Неверное удаление списка");
    }
